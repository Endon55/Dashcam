...
int Muxor::init(int width, int height)
{
    ...
    ret = add_stream(&video_stream, outputContext, &video_codec, fmt->video_codec);
    if(ret < 0)
    {
        spdlog::critical("Failed to add video stream");
        return -1;
    }
    ret = add_stream(&audio_stream, outputContext, &audio_codec, fmt->audio_codec);
    if (ret < 0)
    {
        spdlog::critical("Failed to add video stream");
        return -1;
...
*/
int Muxor::add_stream(OutputStream *stream, AVFormatContext *fmtContext, const AVCodec **codec, enum AVCodecID codec_id)
{
    int ret = 0;
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    switch ((*codec)->type)
    {
        ...
        case AVMEDIA_TYPE_VIDEO:
        {
            ...
            codecContext->gop_size = AV_PIX_FMT_YUV420P;
